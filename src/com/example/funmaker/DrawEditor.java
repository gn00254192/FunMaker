...
public class DrawEditor extends Activity {
	...
	private final int MENU_INFO_GROUP = 7000;
	private final int MENU_INFO_TUTORIAL = 7001;
...
//		requestWindowFeature(Window.FEATURE_CUSTOM_TITLE);
		...
		GuideHelper guideHelper = new GuideHelper(this, 1);
...
//		getWindow().setFeatureInt(Window.FEATURE_CUSTOM_TITLE, R.layout.mytitle);
		...
		Toast.makeText(DrawEditor.this, "你可以在此盡情的創作，並於漫畫中使用", Toast.LENGTH_LONG).show();
		...
		mSCanvas.setFileProcessListener(new FileProcessListener() {
			...
			public void onLoadComplete(boolean bLoadResult) {			 
				if(bLoadResult){
					...
					Toast.makeText(DrawEditor.this, "Load AMS File("+ appID + ") Success!", Toast.LENGTH_LONG).show();
				}
				else{
					Toast.makeText(DrawEditor.this, "Load AMS File Fail!", Toast.LENGTH_LONG).show();				
				}
			}
		});
	...
	private OnLongClickListener mBtnLongClickListener = new OnLongClickListener() {
		...
		public boolean onLongClick(View v) {
			...
			if(nBtnID == mPenBtn.getId()){				
				...
				Toast.makeText(mContext, "圖庫", Toast.LENGTH_LONG).show();
				return true;
			}
...
//				switch (which) {
//				case 0:	// Video file
//					Intent intent = new Intent(DrawEditor.this, SPen_Example_VideoDemoFileList.class);
//					startActivityForResult(intent, REQUEST_CODE_INSERT_VIDEO_OBJECT);					
//					break;
//				case 1: 	// Video URL Link
//					LayoutInflater factory = LayoutInflater.from(DrawEditor.this);
...
//
//
//					AlertDialog dlg = new AlertDialog.Builder(DrawEditor.this)
//					.setTitle("Video URL")
...
//	};
	...
	private void updateModeState(){
		SPenSDKUtils.updateModeState(mSCanvas, null, null, mPenBtn, mEraserBtn, mTextBtn, mFillingBtn, mInsertBtn, mColorPickerBtn, mSelectionModeBtn);
	}	
...
//		}
		...
		else if(requestCode==REQUEST_CODE_INSERT_IMAGE) {
//			Bundle bd = data.getBundleExtra("bundle");
//			String url = bd.getString("bgurl");
//			Log.i("mylog","set bg "+url);
//			mSCanvas.setBGImage(loadBitmap2(url));
...
//					mSCanvas.setBGImage(result);
//					super.onPostExecute(result);
//				}       
//			}.execute(url);
//			BitmapFactory.Options opt = new BitmapFactory.Options();   
//			opt.inJustDecodeBounds = true;  
//			Bitmap bm = BitmapFactory.decodeFile(setting.bgpath, opt);
//			mSCanvas.setBGImage(bm);
			mSCanvas.setBGImagePath(setting.imagenet_selected_image_path);
			Log.i("mylog","executed");
		}
...
//	抓圖片2
	public static Bitmap loadBitmap2(String url) {
		Bitmap bitmap = null;
		try {
			InputStream is = (InputStream)new URL(url).getContent();
			BitmapFactory.Options options = new BitmapFactory.Options();
			options.inJustDecodeBounds = false;
			options.inSampleSize = 2; //width，hight設為原來的二分之一
	...
	boolean saveSAMMFile(String strFileName, boolean bShowSuccessLog){		
		if(mSCanvas.saveSAMMFile(strFileName)){
			if(bShowSuccessLog){
				...
				ImageObject.draweditor_image_str = strFileName;
				Intent intent=new Intent();
				Bundle bundle = new Bundle();
				bundle.putString("path", strFileName);
...
//				finish();
				...
				Log.i("mylog","(DrawEditor)image path = "+ImageObject.draweditor_image_str);
			}
			return true;
		}
		else{
...
//		getMenuInflater().inflate(R.menu.draw_editor, menu);
		
		
		menu.add(MENU_FILE_GROUP,  MENU_FILE_SAVE_AS, 1, "完成");
		menu.add(MENU_INFO_GROUP,  MENU_INFO_TUTORIAL, 2, "說明");
...
//		fileMenu.add(MENU_FILE_GROUP, MENU_FILE_SAVE_AS, 3, "儲存");
...
//	    	SPenSDKUtils.alertActivityFinish(this, "Exit");
		...
		case MENU_FILE_SAVE_AS:
		{
			...
			if(mSCanvas.isCanvasDrawingEmpty(true)){
				Toast.makeText(mContext, "畫布上沒有圖像", Toast.LENGTH_LONG).show();
				return true;
			}
			...
			.setPositiveButton("儲存", new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int whichButton) {
					...
					if(strFileName.length()<=0){
						Toast.makeText(mContext, "儲存失敗，請輸入檔案名稱！", Toast.LENGTH_LONG).show();
						return;
					}
		...
		.setItems(R.array.backgroundimageoperation, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {			
				if(!mSCanvas.filterBGImage(imageOperationByIndex[which], imageOperationLevel))
					Toast.makeText(DrawEditor.this, "To apply backGround image effect, set background image firstly or select valid image effect!!!", Toast.LENGTH_LONG).show();
			}
		})
		.show();
	}
		...
		.setPositiveButton("OK", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int whichButton) {
				...
				if(!mSCanvas.setCheckPreference(mPreferenceCheckedItem))
					Toast.makeText(DrawEditor.this, "Fail to set preference normal!", Toast.LENGTH_LONG).show();
				else
					if(mPreferenceCheckedItem == 0)
						Toast.makeText(DrawEditor.this, "Current Preference = Normal", Toast.LENGTH_LONG).show();
					else if(mPreferenceCheckedItem == 1)
						Toast.makeText(DrawEditor.this, "Current Preference = Favorite", Toast.LENGTH_LONG).show();
					else if(mPreferenceCheckedItem == 2)
						Toast.makeText(DrawEditor.this, "Current Preference = Custom", Toast.LENGTH_LONG).show();
			}
		})
		.setNegativeButton("Cancel", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int whichButton) {
...
//		updateSelectButton();
	...
	public void setSCanvasBG(Bitmap bm) {
		mSCanvas.setBackgroundImage(bm);
	}
