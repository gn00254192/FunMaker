...
import com.example.tools.ToolStampList;
import com.example.funmaker.ImageTextListAdapter.ImageTextListItem;
import com.example.funmaker.R;
import com.example.sammeditor.SPen_Example_AnimationViewer;
import com.example.sammeditor.SPen_Example_SAMMEditor;
import com.example.sammeditor.SPen_Example_VideoDemoFileList;
import com.example.sammeditor.SPen_Example_VoiceRecorder;
import com.example.sammeditor.SPen_Example_SAMMEditor.AlertItem;
import com.example.tools.PreferencesOfAnimationOption;
import com.example.tools.PreferencesOfOtherOption;
import com.example.tools.PreferencesOfSAMMOption;
import com.example.tools.SPenSDKUtils;
import com.example.tools.ToolAudioListView;
import com.example.tools.ToolColorPickerDialog;
import com.example.tools.ToolFileListView;
import com.example.tools.ToolFileTotalInfoShow;
import com.example.tools.ToolListActivity;
import com.example.tools.ToolTextDialogInput;
import com.example.tools.ToolColorPickerDialog.OnColorChangedListener;
...
public class SAMMEditor extends Activity {
	...
	private final int MENU_INSERT_FROM_SD = 5006;
	private final int MENU_INSERT_FROM_LIBRARY = 5007;
	private final int MENU_INSERT_FROM_SELF = 5008;	
	...
	private final int MENU_SHARE_GROUP = 7000;
	private final int MENU_SHARE_FACEBOOK = 7001;
	private final int MENU_SHARE_SELECT = 7002;
	
	private final int MENU_INFO_GROUP = 8000;
	private final int MENU_INFO_TUTORIAL = 8001;
	...
	private final int REQUEST_CODE_OTHER_OPTION = 110;
	private final int REQUEST_CODE_INSERT_STAMP_OBJECT = 112;
	private final int REQUEST_CODE_DRAW_ITEM_OBJECT = 113;
	...
	private final int INSERT_STAMP = 1;
	private final int DRAW_ITEM = 2;
//	private final int INSERT_VIDEO = 3;
...
//	private ImageView		mFillingBtn;
	...
	private final int CANVASVIEW_WIDTH_MARGIN = 0;
	private final int CANVASVIEW_HEIGHT_MARGIN = 0;
	private final int STATIC_CANVAS_WIDTH = 720;
	private final int STATIC_CANVAS_HEIGHT = 1280;
...
//	    this.requestWindowFeature(Window.FEATURE_NO_TITLE);
		...
		GuideHelper guideHelper = new GuideHelper(this, 0);
		...
		mSelectionModeBtn.setOnLongClickListener(mBtnLongClickListener);
...
//		mFillingBtn.setOnLongClickListener(mBtnLongClickListener);
		...
		mInsertBtn.setOnLongClickListener(mBtnLongClickListener);
		mColorPickerBtn = (ImageView) findViewById(R.id.colorPickerBtn);
		mColorPickerBtn.setOnClickListener(mColorPickerListener);
		mColorPickerBtn.setOnLongClickListener(mBtnLongClickListener);
...
//		mCanvasContainer.addView(mSCanvas);
		...
		mBGBitmap = BitmapFactory.decodeResource(getResources(), R.drawable.bg_comic);
...
//				mSCanvas.setSettingViewSizeOption(SCanvasConstants.SCANVAS_SETTINGVIEW_PEN, SCanvasConstants.SCANVAS_SETTINGVIEW_SIZE_UNKNOWN);
		...
		mSCanvas.setFileProcessListener(new FileProcessListener() {
			...
			public void onLoadComplete(boolean bLoadResult) {			 
				if(bLoadResult){
					...
					Toast.makeText(SAMMEditor.this, "Load AMS File("+ appID + ") Success!", Toast.LENGTH_LONG).show();
				}
				else{
					Toast.makeText(SAMMEditor.this, "Load AMS File Fail!", Toast.LENGTH_LONG).show();				
				}
			}
		});
		...
		mSCanvas.setSettingViewShowListener(new SettingViewShowListener() {
			...
			public void onFillingSettingViewShow(boolean bVisible) {
				if(SHOW_LOG){		
					if(bVisible) Log.i(TAG, "Filling setting view is shown");
					else		 Log.i(TAG, "Filling setting view is closed");
				}
			}
		});
...
//		SPenSDKUtils.alertActivityFinish(this, "Exit");
    	...
    	ad.setTitle("確定離開?")
    	.setMessage("離開後將失去進度，請確認已儲存作品。")
    	.setPositiveButton("確定", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int which) {	
				dialog.dismiss();
				SAMMEditor.this.finish();
				finish();
			}
		})
		...
		public boolean onLongClick(View v) {
			...
			if(nBtnID == mPenBtn.getId()){				
				...
				if(!bMovingMode && mSCanvas.getCanvasMode()==SCanvasConstants.SCANVAS_MODE_INPUT_TEXT){
					...
					updateModeState();
					Toast.makeText(mContext, "點選插入文字位置", Toast.LENGTH_LONG).show();
				}
				return true;
			}
			else if(nBtnID == mColorPickerBtn.getId()) {
				Toast.makeText(mContext, "拾色工具，點擊畫布選擇顏色", Toast.LENGTH_LONG).show();
				return true;
			}
			else if(nBtnID == mInsertBtn.getId()) {
				Toast.makeText(mContext, "插入物件", Toast.LENGTH_LONG).show();
				return true;
			}
			else if(nBtnID == mSelectionModeBtn.getId()) {
				Toast.makeText(mContext, "選取工具", Toast.LENGTH_LONG).show();
				return true;
			}
//			else if(nBtnID == mFillingBtn.getId()){
...
//原始AlertDialog
//-----------------------------------------------------------------------
//				String items[] = {"圖像", "圖庫", "創意繪圖"};
//				int images[] = {R.drawable.icon, R.drawable.icon, R.drawable.icon};
//				AlertDialog.Builder ad = new AlertDialog.Builder(mContext);
//				ListView listView = new ListView(SAMMEditor.this);
//				ad.setIcon(getResources().getDrawable(android.R.drawable.ic_input_add));	// Android Resource
//				ad.setTitle("插入")
...
//
//						case INSERT_STAMP:
//							Intent intent_stamp = new Intent(SAMMEditor.this, com.example.tools.ToolStampList.class);
//							startActivityForResult(intent_stamp, REQUEST_CODE_INSERT_STAMP_OBJECT);	
//							break;
//						case DRAW_ITEM:
//							Intent intent_draw = new Intent(SAMMEditor.this, DrawEditor.class);
//							startActivityForResult(intent_draw, REQUEST_CODE_DRAW_ITEM_OBJECT);
//							break;
////						case INSERT_VIDEO:
...
//---------------------------------------------------------------------
//				把資料加入ArrayList中
				String mItems[] = {"圖像", "圖庫", "自己畫"};
				int mItem_imgs[] = {R.drawable.image_icon, R.drawable.icon, R.drawable.draw_icon};
				ArrayList<HashMap<String,Object>> list = new ArrayList<HashMap<String,Object>>();
				for(int i=0;i<mItems.length;i++) {
					HashMap<String,Object> item = new HashMap<String,Object>();
					item.put("icon", mItem_imgs[i]);
					item.put("items", mItems[i]);
					list.add(item);
				}
//				新增SimpleAdapter
				SimpleAdapter adapter = new SimpleAdapter(SAMMEditor.this, list, R.layout.list_item_image_text, new String[] {"icon", "items"}, new int[] {R.id.list_img, R.id.list_text});	
				AlertDialog.Builder ad = new AlertDialog.Builder(SAMMEditor.this);
	            ad.setIcon(getResources().getDrawable(android.R.drawable.ic_input_add));
	            ad.setTitle("插入");
	            final ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(SAMMEditor.this, android.R.layout.select_dialog_item);
	            arrayAdapter.add("圖像");
	            arrayAdapter.add("圖庫");
	            arrayAdapter.add("自己畫");
...
//				switch (which) {
//				case 0:	// Video file
//					Intent intent = new Intent(SAMMEditor.this, SPen_Example_VideoDemoFileList.class);
//					startActivityForResult(intent, REQUEST_CODE_INSERT_VIDEO_OBJECT);					
//					break;
//				case 1: 	// Video URL Link
//					LayoutInflater factory = LayoutInflater.from(SAMMEditor.this);
...
//
//
//					AlertDialog dlg = new AlertDialog.Builder(SAMMEditor.this)
//					.setTitle("Video URL")
...
//	}
	...
	private void updateModeState(){
		SPenSDKUtils.updateModeState(mSCanvas, null, null, mPenBtn, mEraserBtn, mTextBtn, mInsertBtn, mColorPickerBtn, mPlayBtn, mSelectionModeBtn);
	}	
...
//			}
//		}
		else if(requestCode == REQUEST_CODE_INSERT_STAMP_OBJECT) {    			
		...
		else if(requestCode==REQUEST_CODE_DRAW_ITEM_OBJECT) {
...
//			String imagePath = SPenSDKUtils.getRealPathFromURI(this, imageFileUri);
			...
			String imagePath = ImageObject.draweditor_image_str;
			Log.i("mylog","(SAMMEditor)image path = "+imagePath);
...
//		fileMenu.add(MENU_FILE_GROUP, MENU_FILE_NEW, 1, "開新檔案");
		fileMenu.add(MENU_FILE_GROUP, MENU_FILE_LOAD, 2, "載入檔案");
		fileMenu.add(MENU_FILE_GROUP, MENU_FILE_SAVE_AS, 3, "儲存檔案");
...
//		fileMenu.add(MENU_FILE_GROUP, MENU_FILE_PLAY_NORMAL, 5, "Animation");
		...
		SubMenu insertMenu = menu.addSubMenu("加入圖片");
		insertMenu.add(MENU_INSERT_GROUP, MENU_INSERT_FROM_SELF, 1, "自己畫");
		insertMenu.add(MENU_INSERT_GROUP, MENU_INSERT_FROM_SD, 2, "從手機加入");
		insertMenu.add(MENU_INSERT_GROUP, MENU_INSERT_FROM_LIBRARY, 3, "從圖庫加入");
		
//		SubMenu editMenu = menu.addSubMenu("編輯");
//		editMenu.add(MENU_EDIT_GROUP, MENU_EDIT_DELETE_OBJECT, 10, "刪除物件");
//		editMenu.add(MENU_EDIT_GROUP, MENU_EDIT_ROTATE_DEGREE, 20, "旋轉物件");
...
//		moreMenu.add(MENU_MORE_GROUP, MENU_MORE_OTHER_OPTION, 5, "Other Option");
		...
		menu.add(MENU_SHARE_GROUP, MENU_SHARE_SELECT, 3, "分享");
		menu.add(MENU_INFO_GROUP, MENU_INFO_TUTORIAL, 4, "說明");
	...
	public boolean onOptionsItemSelected(MenuItem item)	{
		...
		// Insert Menu
		//================================================
		case MENU_INSERT_FROM_SD:
		{
			callGalleryForInputImage(REQUEST_CODE_INSERT_IMAGE_OBJECT);
			break;
		}
		case MENU_INSERT_FROM_LIBRARY:
		{
			Intent intent_stamp = new Intent(SAMMEditor.this, com.example.tools.ToolStampList.class);
			startActivityForResult(intent_stamp, REQUEST_CODE_INSERT_STAMP_OBJECT);	
			break;
		}
		case MENU_INSERT_FROM_SELF:
		{
			Intent intent_draw = new Intent(SAMMEditor.this, DrawEditor.class);
			startActivityForResult(intent_draw, REQUEST_CODE_DRAW_ITEM_OBJECT);
//			mSCanvas.closeSCanvasView();
		...
		{
			setting.upload=mSCanvas.getCanvasBitmap(false);
			Intent intent = new Intent(this, FacebookShare.class);
			startActivity(intent);	
		}
		break;	
		case MENU_SHARE_SELECT:
		{
            ...
            ImageObject.share_img_path = strPath+"tmp_share.png";
			File myDrawFile = new File(ImageObject.share_img_path);
			try {
				BufferedOutputStream bos = new BufferedOutputStream
				(new FileOutputStream(myDrawFile));
				mSCanvas.getBitmap(false).compress(Bitmap.CompressFormat.PNG, 90, bos);
//				imageids[position].compress(Bitmap.CompressFormat.PNG, 90, bos);
			...
			Log.d(null, "Save file ok! Path = "+ImageObject.share_img_path);
			
			
			Intent shareIntent = new Intent(Intent.ACTION_SEND);  
	        shareIntent.setType("image/*");  
	        String ImagePath = "file://" + ImageObject.share_img_path;
	        Uri uri = Uri.parse(ImagePath);
//	        Uri uri = Uri.fromFile(getFileStreamPath("tmp_share.png"));  
	        shareIntent.putExtra(Intent.EXTRA_STREAM, uri);  
	        startActivity(Intent.createChooser(shareIntent, "請選擇"));  
		}
		break;
		case MENU_INFO_TUTORIAL:
		{
			GuideHelper guideHelper = new GuideHelper(this, 0);
	        guideHelper.openGuide();
		}
...
//					canvasOption.mSAMMOption.setContentsQuality(PreferencesOfSAMMOption.getPreferenceSaveImageQuality(mContext));
					canvasOption.mSAMMOption.setContentsQuality(SOptionSAMM.SAMM_CONTENTS_QUALITY_ORIGINAL);
		...
		.setItems(R.array.backgroundimageoperation, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {			
				if(!mSCanvas.filterBGImage(imageOperationByIndex[which], imageOperationLevel))
					Toast.makeText(SAMMEditor.this, "To apply backGround image effect, set background image firstly or select valid image effect!!!", Toast.LENGTH_LONG).show();
			}
		})
		.show();
	}
		...
		.setPositiveButton("OK", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int whichButton) {
				...
				if(!mSCanvas.setCheckPreference(mPreferenceCheckedItem))
					Toast.makeText(SAMMEditor.this, "Fail to set preference normal!", Toast.LENGTH_LONG).show();
				else
					if(mPreferenceCheckedItem == 0)
						Toast.makeText(SAMMEditor.this, "Current Preference = Normal", Toast.LENGTH_LONG).show();
					else if(mPreferenceCheckedItem == 1)
						Toast.makeText(SAMMEditor.this, "Current Preference = Favorite", Toast.LENGTH_LONG).show();
					else if(mPreferenceCheckedItem == 2)
						Toast.makeText(SAMMEditor.this, "Current Preference = Custom", Toast.LENGTH_LONG).show();
			}
		})
		.setNegativeButton("Cancel", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int whichButton) {
